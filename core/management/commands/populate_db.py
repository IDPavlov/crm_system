...
#                 order_date = datetime.now() - timedelta(days=random.randint(0, 500))
#
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        with transaction.atomic():
            # Удаление старых данных (осталось без изменений)
            User.objects.filter(is_superuser=False).delete()
            Client.objects.all().delete()
            Product.objects.all().delete()
            Order.objects.all().delete()
            ...
            # Создание групп и пользователей (осталось без изменений)
            # Получаем или создаем группы
            managers_group, _ = Group.objects.get_or_create(name='Managers')
            analysts_group, _ = Group.objects.get_or_create(name='Analysts')
            ...
            for i in range(1, 21):
                ...
                products.append(product)  # Важно: добавляем КАЖДЫЙ товар

            # Создание заказов
            for i in range(1, 101):
                client = random.choice(clients)
                # Генерация случайной даты для КАЖДОГО заказа
                order_date = datetime.now() - timedelta(days=random.randint(0, 500))
                ...
                order = Order.objects.create(
                    ...
                    status=random.choice(['pending', 'paid', 'shipped', 'delivered', 'cancelled']),
                    total_amount=0,
                    created_at=order_date  # Используем сгенерированную дату
                )
